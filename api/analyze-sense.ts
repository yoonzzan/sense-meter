  ...
  try {
    ...
    if (hasSignificantGap) {
      ...
      gapAnalysisPromptSection = `
        ...
        *   **시장 기회 발견**:\n\n(두 번 줄바꿈 후 내용 작성) ${disagreePercentage.toFixed(0)}%의 비공감(혹은 공감) 데이터에서 발견할 수 있는 새로운 니치 마켓이나 차별화 포인트는 무엇인가요?
      ...
      gapAnalysisPromptSection = `
        **3. gapAnalysis (종합 인사이트 및 트렌드):**
        *   **메가 트렌드 연결**:\n\n(두 번 줄바꿈 후 내용 작성) 대중적인 공감을 얻은 이 감각이 현재의 어떤 메가 트렌드와 맞닿아 있는지 분석해주세요.
        *   **아이디어 제안 (Provider vs User)**:\n\n(두 번 줄바꿈 후 내용 작성)
...
${commentsText}
        ...
        *   **핵심 타겟 정의**:\n\n(두 번 줄바꿈 후 내용 작성) 이 감각에 반응하는 핵심 타겟 그룹을 정의하고 그들의 특징(라이프스타일, 가치관)을 한 문장으로 요약해주세요.
        *   **서비스 적용 포인트**:\n\n(두 번 줄바꿈 후 내용 작성) 이들이 좋아하는 요소(반응 태그/댓글 참고)를 실제 서비스나 제품에 어떻게 녹여낼 수 있을지 구체적인 아이디어를 제안해주세요.
        *   *주의: '1. 공감하는 시선' 같은 대분류 제목은 포함하지 마세요.*

        **2. disagreeAnalysis (다른 시선):**
        *   **비공감 원인 분석 (Pain Point)**:\n\n(두 번 줄바꿈 후 내용 작성) 공감하지 못하는 그룹이 느끼는 '불편함'이나 '괴리감'의 원인은 무엇인가요? (댓글의 반대 의견 참고)
