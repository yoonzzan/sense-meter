...
app.post('/api/analyze-sense', async (req, res) => {
    const apiKey = process.env.GEMINI_API_KEY || process.env.API_KEY;

    if (!apiKey) {
        console.warn('API key is missing. Returning mock response for testing.');
        // Mock response for testing
        return res.status(200).json({
            agree: "이 경험은 정말 특별하네요! (Mock Analysis)\n사용자님은 작은 것에서도 큰 의미를 찾는 섬세한 분이신 것 같아요.",
            disagree: "하지만 어떤 사람들은 이 상황을 다르게 볼 수도 있겠어요. (Mock Analysis)\n실용성을 중시하는 사람이라면 이 부분이 이해되지 않을 수 있죠.",
            gapAnalysis: "이 간극은 서로 다른 가치관에서 비롯된 것 같아요. (Mock Analysis)\n하지만 걱정 마세요, 당신의 감각은 그 자체로 소중하니까요."
        });
    }
